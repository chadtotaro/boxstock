  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1.5 shrink-0">
        <button onClick={onUndo} disabled={!canUndo} title="Undo (Ctrl+Z)" className="flex items-center justify-center w-8 h-8 rounded transition-all cursor-pointer disabled:cursor-default" style={{ color: canUndo ? 'var(--c-text-icon)' : 'var(--c-text-icon-disabled)', backgroundColor: 'transparent' }} onMouseEnter={(e) => { if (canUndo) { e.currentTarget.style.backgroundColor = 'var(--c-accent-bg-hover)'; e.currentTarget.style.color = 'var(--c-accent)'; } }} onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = 'transparent'; e.currentTarget.style.color = canUndo ? 'var(--c-text-icon)' : 'var(--c-text-icon-disabled)'; }}><Icon path={mdiUndo} size={0.75} /></button>
        <button onClick={onRedo} disabled={!canRedo} title="Redo (Ctrl+Shift+Z)" className="flex items-center justify-center w-8 h-8 rounded transition-all cursor-pointer disabled:cursor-default" style={{ color: canRedo ? 'var(--c-text-icon)' : 'var(--c-text-icon-disabled)', backgroundColor: 'transparent' }} onMouseEnter={(e) => { if (canRedo) { e.currentTarget.style.backgroundColor = 'var(--c-accent-bg-hover)'; e.currentTarget.style.color = 'var(--c-accent)'; } }} onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = 'transparent'; e.currentTarget.style.color = canRedo ? 'var(--c-text-icon)' : 'var(--c-text-icon-disabled)'; }}><Icon path={mdiRedo} size={0.75} /></button>
