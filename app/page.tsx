...
function TrackCard({ name, tiles, room, thumbnail, onClick, onDelete }: {
  ...
  return (
    <div
      ...
      style={{ borderRadius: 8, overflow: 'visible', cursor: onClick ? 'pointer' : 'default', border: hovered ? '2px solid #0fc4ba' : '2px solid transparent', transition: 'border-color 0.15s' }}
    >
      <div style={{ height: 162, backgroundColor: '#000', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', borderRadius: '6px 6px 0 0', overflow: 'hidden' }}>
        {thumbnail ? <img src={thumbnail} alt={name} style={{ width: '100%', height: '100%', objectFit: 'contain' }} /> : <Layers size={40} style={{ color: 'rgba(255,255,255,0.1)' }} />}
        {onDelete && (
      ...
      <div style={{ backgroundColor: '#fff', padding: '12px 14px 16px', borderRadius: '0 0 6px 6px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          <span style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 13, color: '#000' }}><Icon path={mdiGrid} size={0.65} />{tiles}</span>
          {room && room !== '-- x --' && (<span style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 12, color: '#000' }}><RulerIcon size={16} />{room}</span>)}
        </div>
      </div>
    </div>
  );
